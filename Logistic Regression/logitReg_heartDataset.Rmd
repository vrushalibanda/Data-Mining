...
### Get the dataset from the UCI depository
```{r}
...
#### Reformat the data so it would be
1-Easy to use (add nice column names)
2-Interpret correctly by glm()..
```{r}
colnames(data) <- c(
  "age",
  "sex",# 0 = female, 1 = male
...
str(data)
```
There are  columns that need to be converted to factors and "?" characters that need to converted to NA:
```{r}
data[data == "?"] <- NA
str(data)
...
str(data)
```
Convert "hd" to a more readable "healthy" and "unhealthy"
```{r}
...
Missing data
```{r}
## Now determine how many rows have "NA", Missing data". If it's just
## a few, we can remove them from the dataset, otherwise we should consider
## imputing the values with a KNN, bagged trees or some other imputation method.
nrow(data[is.na(data$ca) | is.na(data$thal),])
data[is.na(data$ca) | is.na(data$thal),]
```
Since only 2% (6/303) of observations are missing, we can remove them.
```{r}
nrow(data)
...
```
### Simple logistic regression that uses sex to predict heart disease:
```{r}
...
Regression:
```{r}
heart_logit1 <- glm(hd ~ sex, data=data, family="binomial")
summary(heart_logit1)
```
...
female.log.odds <- log(25 / 71)  #unheathy and heathy female
female.log.odds   #same like coefficient in above chunk
```
Second coefficient
```{r}
...
ll.null <- heart_logit1$null.deviance/-2
ll.proposed <- heart_logit1$deviance/-2
...
1 - pchisq((heart_logit1$null.deviance - heart_logit1$deviance), df=1)


#getting differnt value doubut
```
...
predicted.data <- data.frame(
  probability.of.hd=heart_logit1$fitted.values,
  sex=data$sex)
...
```{r}
heart_logit_all <- glm(hd ~ ., data=data, family="binomial")
summary(heart_logit_all)
 
## Now calculate the overall "Pseudo R-squared" and its p-value
ll.null <- heart_logit_all$null.deviance/-2
ll.proposed <- heart_logit_all$deviance/-2
 
## McFadden's Pseudo R^2 = [ LL(Null) - LL(Proposed) ] / LL(Null)
(ll.null - ll.proposed) / ll.null
 
## The p-value for the R^2
1 - pchisq(2*(ll.proposed - ll.null), df=(length(heart_logit_all$coefficients)-1))
 
## now we can plot the data
predicted.data <- data.frame(
  probability.of.hd=heart_logit_all$fitted.values,
  hd=data$hd)
